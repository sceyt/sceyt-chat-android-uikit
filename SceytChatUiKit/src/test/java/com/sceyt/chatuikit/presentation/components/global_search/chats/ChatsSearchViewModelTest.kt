...
class ChatsSearchViewModelTest {
    ...
    // ─── Blank query (recent chats) ───────────────────────────────────────────────

    @Test
    fun `blank query first page loads recent chats`() = runTest(dispatcher) {
        val dataSource = ChatsFakeDataSource()
        dataSource.enqueueChats(fakeChatPage(count = 5, hasMore = false))

        val vm = vmForTab(GlobalSearchTab.Chats, dataSource)
        advanceUntilIdle()
    ...
    @Test
    fun `blank query offset equals loaded chat count, not listItems size`() = runTest(dispatcher) {
        val dataSource = ChatsFakeDataSource()
        dataSource.enqueueChats(fakeChatPage(count = 5, hasMore = true))

        val vm = vmForTab(GlobalSearchTab.Chats, dataSource)
        advanceUntilIdle()

        // listItems has header + 5 chats = 6, but offset must be 5 (data count only)
    ...
    fun `blank query loadMore passes state offset as data source offset`() = runTest(dispatcher) {
        val dataSource = ChatsFakeDataSource()
        dataSource.enqueueChats(fakeChatPage(count = 5, hasMore = true))
        dataSource.enqueueChats(fakeChatPage(count = 3, hasMore = false))
        ...
        assertThat(dataSource.chatCalls[1].offset).isEqualTo(5)
    }

    @Test
    fun `blank query loadMore appends chats and advances offset`() = runTest(dispatcher) {
        val dataSource = ChatsFakeDataSource()
        dataSource.enqueueChats(fakeChatPage(count = 5, hasMore = true))
        dataSource.enqueueChats(fakeChatPage(count = 3, hasMore = false))
    ...
    fun `blank query loadMore is no-op when hasMore is false`() = runTest(dispatcher) {
        ...
        assertThat(dataSource.chatCalls).hasSize(1)
    }
    ...
    fun `selected member first page fetches messages with correct senderId`() = runTest(dispatcher) {
        ...
        val vm = vmForState(
            GlobalSearchSessionState(
                activeTab = GlobalSearchTab.Chats,
    ...
    fun `typed query first page loads both channels and messages`() = runTest(dispatcher) {
        val dataSource = ChatsFakeDataSource()
        dataSource.enqueueChats(fakeChatPage(count = 2, hasMore = false))
        dataSource.enqueueMessages(fakeMessagePage(count = 3, hasMore = true))

        val vm = vmForState(
            GlobalSearchSessionState(activeTab = GlobalSearchTab.Chats, query = "hello"),
            dataSource
        )
        advanceUntilIdle()
    ...
    fun `typed query hasMore reflects messages hasMore, not false`() = runTest(dispatcher) {
        val dataSource = ChatsFakeDataSource()
        dataSource.enqueueChats(fakeChatPage(count = 1, hasMore = false))
    ...
    fun `typed query loadMore fetches messages only, no second channels call`() = runTest(dispatcher) {
        ...
        // Only 1 chats call (first page), messages called twice
        assertThat(dataSource.chatCalls).hasSize(1)
    ...
    fun `typed query new query resets offset and reloads from zero`() = runTest(dispatcher) {
        ...
        val session = GlobalSearchSessionStore(
            GlobalSearchSessionState(activeTab = GlobalSearchTab.Chats, query = "first")
        )
        ...
        val state = dataSource.messageCalls
        assertThat(state[0].offset).isEqualTo(0)
        assertThat(state[1].offset).isEqualTo(0)
    }
    ...
    fun `typed query shorter than min length does not fetch messages`() = runTest(dispatcher) {
        ...
        val vm = vmForState(
            GlobalSearchSessionState(activeTab = GlobalSearchTab.Chats, query = "h"), // length 1 < min 2
            dataSource
        )
        advanceUntilIdle()
    ...
    private fun vmForTab(tab: GlobalSearchTab, dataSource: ChatsFakeDataSource) =
        vmForState(GlobalSearchSessionState(activeTab = tab), dataSource)

    private fun vmForState(state: GlobalSearchSessionState, dataSource: ChatsFakeDataSource) =
        ChatsSearchViewModel(GlobalSearchSessionStore(state), dataSource, dispatcher)
}

// ─── Fake data source ─────────────────────────────────────────────────────────

private class ChatsFakeDataSource : GlobalSearchDataSource {
    private val chatQueue = ArrayDeque<GlobalSearchPage<SceytChannel>>()
    private val messageQueue = ArrayDeque<GlobalSearchPage<GlobalSearchMessageResult>>()

    val chatCalls = mutableListOf<ChatsCall>()
    val messageCalls = mutableListOf<MessagesCall>()

    fun enqueueChats(page: GlobalSearchPage<SceytChannel>) = chatQueue.addLast(page)
    ...
    override suspend fun getRecentChats(offset: Int, limit: Int): GlobalSearchPage<SceytChannel> {
        chatCalls += ChatsCall(query = null, offset = offset)
        return chatQueue.removeFirstOrNull() ?: GlobalSearchPage(emptyList(), false)
    }

    override suspend fun searchChats(query: String, offset: Int, limit: Int): GlobalSearchPage<SceytChannel> {
        chatCalls += ChatsCall(query = query, offset = offset)
        return chatQueue.removeFirstOrNull() ?: GlobalSearchPage(emptyList(), false)
    }
...
private data class ChatsCall(val query: String?, val offset: Int)
private data class MessagesCall(val query: String, val senderId: String?, val offset: Int)

// ─── Factories ────────────────────────────────────────────────────────────────

private fun fakeChatPage(count: Int, hasMore: Boolean) =
