...
data class LinksSearchPageStyle(
    @param:ColorInt val backgroundColor: Int,
    @param:LayoutRes val emptyState: Int,
    val emptyStateStyle: EmptyStateStyle,
    val dateSeparatorStyle: DateSeparatorStyle,
    val linkItemStyle: ChannelInfoLinkItemStyle,
) : SceytComponentStyle() {

    companion object {
        var styleCustomizer = StyleCustomizer<LinksSearchPageStyle> { _, style -> style }
    }

    internal class Builder(private val context: Context) {
        fun build(): LinksSearchPageStyle {
            return LinksSearchPageStyle(
                ...
                linkItemStyle = ChannelInfoLinkItemStyle.Builder(context, null).build(),
