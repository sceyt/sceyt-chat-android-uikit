package com.sceyt.chatuikit.presentation.uicomponents.startchat
...
class SceytStartChatActivity : AppCompatActivity() {
    ...
    private fun initViews() {
        ...
        binding.tvNewGroup.setOnClickListener {
            addMembersActivityLauncher.launch(SceytAddMembersActivity.newInstance(this), animOptions)
        }

        binding.tvNewChannel.setOnClickListener {
            createConversationLauncher.launch(Intent(this, SceytCreateChannelActivity::class.java), animOptions)
        }
    }
    ...
    private val addMembersActivityLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        if (result.resultCode == Activity.RESULT_OK) {
            result.data?.parcelableArrayList<SceytMember>(SceytAddMembersActivity.SELECTED_USERS)?.let { members ->
                createGroupLauncher.launch(SceytCreateGroupActivity.newIntent(this, members), animOptions)
            }
        }
    }
    ...
    companion object {
        ...
        fun launch(context: Context) {
            context.launchActivity<SceytStartChatActivity>(com.sceyt.chatuikit.R.anim.sceyt_anim_slide_in_right, sceyt_anim_slide_hold)
        }
    }
}