...
class FindOrCreateChanelByUriTests : SceytKoinComponent {
    ...
    fun setUp() {
        SceytChatUIKit.initialize(
            ...
            appId = "yzr58x11rm",
            apiUrl = "https://mp-api-staging-htgcloud-region-02.waafi.com",
        ...
        token = fetchToken(currentUser.id)
    }

    private fun fetchToken(userId: String): String {
        val url =
            "https://hm25ehfh6i.execute-api.eu-central-1.amazonaws.com/load-test/user/genToken?user=$userId"
    ...
    @Test
    fun findOrCreatePendingChannelByUri_should_create_new_channel_if_channel_not_found() =
        runBlocking {
            ChatClient.getClient().connect(token)
            awaitToConnectSceytWithTimeout(10_000L) // real time inside IO
            ...
            if (!response.assertIfError("Error getting channel by uri")) {
                if (response.data == null) {
                    ...
                    when (val result = channelLogic.findOrCreatePendingChannelByUri(data)) {
                        is SceytResponse.Error -> Truth.assertWithMessage("Error creating channel: ${result.message}")
                            .fail()
    ...
    @Test
    fun findOrCreatePendingChannelByUri_should_not_create_new_channel_if_channel_already_exists() =
        runBlocking {
            ChatClient.getClient().connect(token)
