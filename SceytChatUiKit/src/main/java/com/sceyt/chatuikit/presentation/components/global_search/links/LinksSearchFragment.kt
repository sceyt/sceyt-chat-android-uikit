...
import com.sceyt.chatuikit.databinding.SceytFragmentLinksSearchBinding
...
import com.sceyt.chatuikit.presentation.components.global_search.links.adapter.LinksSearchListAdapter
import com.sceyt.chatuikit.presentation.components.global_search.links.adapter.LinksSearchViewHolderFactory
...
open class LinksSearchFragment : Fragment(R.layout.sceyt_fragment_links_search) {
    protected lateinit var session: GlobalSearchSession
    protected lateinit var style: GlobalSearchStyle
    protected open val viewModel: LinksSearchViewModel by viewModels {
        createViewModelFactory(session)
    }

    private var _binding: SceytFragmentLinksSearchBinding? = null
    protected val binding: SceytFragmentLinksSearchBinding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        _binding = SceytFragmentLinksSearchBinding.bind(view)
        applyStyle()
        setupRecyclerView()
        observeState()
    }
    ...
    protected open fun createViewModelFactory(session: GlobalSearchSession): ViewModelProvider.Factory {
        return LinksSearchViewModelFactory(session)
    }

    protected open fun createListFactory(): LinksSearchViewHolderFactory {
        return LinksSearchViewHolderFactory(
            context = requireContext(),
            style = style.linksPageStyle,
            needMediaDataCallback = viewModel::onNeedMediaInfo,
            onAttachmentClick = { item -> onAttachmentClicked(item.result) },
        )
    }

    protected open fun createListAdapter(): LinksSearchListAdapter {
        return LinksSearchListAdapter(
    ...
    protected open fun render(state: LinksSearchState) {
    ...
    protected open val hostActivity: GlobalSearchActivity
        get() = requireActivity() as? GlobalSearchActivity
            ?: error("LinksSearchFragment must be hosted by GlobalSearchActivity.")
    ...
    protected open fun applyStyle() {
        binding.root.setBackgroundColor(style.linksPageStyle.backgroundColor)
        binding.pageStateView.setPageStatesView(style.linksPageStyle)
    }
...
private fun LinksSearchState.toPageState(): PageState = when {
