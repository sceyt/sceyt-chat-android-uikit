...
internal class HandleMessagesInRangeUseCase(
    ...
    suspend operator fun invoke(
        ...
        Log.d(tag, "Querying local DB for messages in range [$startId, $endId]")

        // Get all message IDs from local database in the range
        val localIds = messageDao.getMessagesIdsByRangeIgnoreUnlist(
            channelId = channelId,
            startId = startId,
            endId = endId,
        ...
        if (deletedMessageIds.isNotEmpty()) {
            ...
            if (count > 0) {
                ...
                SceytLog.d(
                    tag, "Successfully deleted $count messages from DB and cache " +
                            "(startId=$startId, endId=$endId, syncStartTime=$syncStartTime, channelId=$channelId)"
                )
            }
        } else {
            Log.d(tag, "No deleted messages found in range $startId-$endId")
        }
    }
}
