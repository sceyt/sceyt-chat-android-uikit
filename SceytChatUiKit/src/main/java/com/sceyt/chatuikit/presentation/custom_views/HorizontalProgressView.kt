...
class HorizontalProgressView @JvmOverloads constructor(
    ...
    init {
        context.obtainStyledAttributes(
            ...
            progress = typedArray.getInt(
                R.styleable.HorizontalProgressView_horizontalProgressViewProgress,
                0
            )
    ...
    fun setProgress(value: Int, animate: Boolean = false, duration: Long = 350) {
        if (animate) {
    ...
    private fun animateProgress(from: Int, to: Int, duration: Long) {
        ...
        progressAnimator = ValueAnimator.ofFloat(from.toFloat(), to.toFloat()).apply {
            ...
            addUpdateListener { animator ->
                currentProgress = (animator.animatedValue as Float).coerceIn(0f, 100f)
                invalidate()
            }
            start()
        }
