    ...
    override fun compare(next: MessageListItem, prev: MessageListItem): Int {
        return when {
            (next is MessageListItem.MessageItem) && next.message.deliveryStatus == DeliveryStatus.Pending
                    || (prev is MessageListItem.MessageItem) && prev.message.deliveryStatus == DeliveryStatus.Pending -> 0

            (next is MessageListItem.LoadingMoreItem) || (prev is MessageListItem.LoadingMoreItem) -> 0
