...
interface PersistenceMessagesLogic {
    suspend fun onMessage(
        ...
        sendDeliveryMarker: Boolean = true
    )
    ...
    suspend fun sendFrowardMessages(
        channelId: Long,
        vararg messageToSend: Message
    ): SceytResponse<Boolean>

    suspend fun sendMessageWithUploadedAttachments(
        channelId: Long,
        message: Message,
    ...
    suspend fun getMessageFromServerById(
        channelId: Long,
        messageId: Long
    ): SceytResponse<SceytMessage>
